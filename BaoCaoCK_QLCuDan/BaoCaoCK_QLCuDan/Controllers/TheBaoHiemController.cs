...
namespace BaoCaoCK_QLCuDan.Controllers
{
    public class TheBaoHiemController : Controller
    {
        ...
        public async Task<ActionResult> Create()
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                try
                {
                        ...
                        // LƯU Ý: Mặc định API chỉ trả về 10 người (PageSize=10). 
                        // Nếu muốn lấy hết cho Dropdown, hãy tăng pageSize lên (ví dụ 1000)
                        HttpResponseMessage Res = await client.GetAsync("api/CuDans?pageSize=1000");

                        if (Res.IsSuccessStatusCode)
                        {
                            // Sửa lỗi 1: Dùng await thay vì .Result để tránh treo máy (Deadlock)
                            var response = await Res.Content.ReadAsStringAsync();

                            // Sửa lỗi 2: Deserialize vào Object Wrapper thay vì List trực tiếp
                            var apiData = JsonConvert.DeserializeObject<CuDanApiResponse>(response);

                            // Lấy danh sách thật sự từ thuộc tính .Items
                            if (apiData != null && apiData.Items != null)
                            {
                                listCuDan = apiData.Items;
                            }
                        }
                ...
                {
                    // Ghi log lỗi nếu cần, không để web bị sập
                    System.Diagnostics.Debug.WriteLine("Lỗi load API Cư dân: " + ex.Message);
                }
            }
        ...
        {
            // 1. Khởi tạo list rỗng để tránh lỗi null
            List<CuDan> listCuDan = new List<CuDan>();
            ...
            // Tạo SelectList (Dropdown)
            ViewBag.MaCuDan = new SelectList(listCuDan, "MaCuDan", "HoTen", selectedId);
        }
        public class CuDanApiResponse
        {
            public int TotalItems { get; set; }

            // Khớp với Page trong API
            public int Page { get; set; }

            // Khớp với PageSize trong API
            public int PageSize { get; set; }

            // Khớp với Items trong API
            // Đây mới là cái list bạn cần lấy ra
