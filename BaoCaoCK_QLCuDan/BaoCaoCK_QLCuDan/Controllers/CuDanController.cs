...
namespace BaoCaoCK_QLCuDan.Controllers
{
    public class CuDanController : Controller
    {
        ...
        // =============================================
        // ACTION: XEM PROFILE (MY PROFILE)
        // =============================================
        public ActionResult MyProfile()
        {
            try
            {
                var token = Session["AccessToken"] as string;
                ...
                using (var client = CreateClient())
                {
                    ...
                    // Gọi API (Đồng bộ)
                    var response = client.GetAsync("api/CuDans/me").Result;

                    if (response.IsSuccessStatusCode)
                    {
                        cuDan = response.Content.ReadAsAsync<CuDan>().Result;
                    }
                    else
                    {
                ...
                if (cuDan == null)
                {
                    // --- CHẾ ĐỘ DEBUG: IN LỖI RA MÀN HÌNH ---
                    return Content($"[LỖI] Không thể tải hồ sơ cư dân.<br/>" +
                                   $"Token hiện tại: {token.Substring(0, 15)}...<br/>" +
                                   $"Lỗi từ API: {errorMsg}<br/>" +
                                   $"Hãy kiểm tra lại: <br/>" +
                                   $"1. Trong Database bảng CuDan đã có dòng nào chứa AccountId khớp với User chưa?<br/>" +
                                   $"2. API bên Swagger có chạy được api/CuDans/me không?");
                }

                // Cập nhật lại Session đề phòng
            ...
            {
                return Content($"[LỖI EXCEPTION]: {ex.Message} <br/> {ex.StackTrace}");
            }
        }

        // =============================================
        // ACTION: HIỆN FORM SỬA (EDIT PROFILE)
        // =============================================
        public ActionResult EditMyProfile(int? id)
        {
            ...
            var cuDan = GetCuDanFromAPI(); // Lấy lại thông tin mới nhất để điền vào form

            if (cuDan == null) return RedirectToAction("MyProfile");

            return View(cuDan);
        }

        // =============================================
        // ACTION: XỬ LÝ CẬP NHẬT (POST)
        // =============================================
        [HttpPost]
        [ValidateInput(false)] // Cho phép gửi HTML (nếu dùng editor)
        [ValidateAntiForgeryToken]
        public ActionResult UpdateMyProfile(CuDan model, HttpPostedFileBase ImageFile)
        {
            var token = Session["AccessToken"] as string;
            ...
            if (ImageFile != null && ImageFile.ContentLength > 0)
            {
                ...
                Session["UserAvatar"] = model.Avatar; // Cập nhật lại Session hiển thị
            }

            // 2. Gọi hàm cập nhật API
            bool result = UpdateCuDanToAPI(model);
            ...
            else
            {
                ...
                return View("EditMyProfile", model);
            }
        }

        // =============================================
        // HÀM HỖ TRỢ GỌI API (IMPLEMENT CHI TIẾT)
        // =============================================

        // Lấy thông tin cư dân (Dùng API "me" để tự động lấy theo Token)
        private CuDan GetCuDanFromAPI()
        {
            var token = Session["AccessToken"] as string;
            using (var client = CreateClient())
            {
                ...
                // Gọi API api/CuDans/me [cite: 137]
                // Dùng .Result để chạy đồng bộ (Synchronous) trong Action MVC thường
                var response = client.GetAsync("api/CuDans/me").Result;

                if (response.IsSuccessStatusCode)
                {
                    return response.Content.ReadAsAsync<CuDan>().Result;
                }
                return null;
            }
        }
        ...
        // Cập nhật thông tin
        private bool UpdateCuDanToAPI(CuDan model)
        {
            var token = Session["AccessToken"] as string;
            using (var client = CreateClient())
            {
                ...
                // Gọi API PUT api/CuDans/me [cite: 137]
                var response = client.PutAsJsonAsync("api/CuDans/me", model).Result;
