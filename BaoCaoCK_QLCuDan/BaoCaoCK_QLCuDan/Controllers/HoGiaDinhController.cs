...
namespace BaoCaoCK_QLCuDan.Controllers
{
    public class HoGiaDinhController : Controller
    {
        ...
        public async Task<ActionResult> Details(int? id, int page = 1)
        {
            ...
            // Lấy danh sách thành viên có phân trang (4 thành viên mỗi trang)
            if (model != null)
            {
                using (var client = new HttpClient())
                {
                    var membersResponse = await client.GetAsync($"{apiBaseUrl}/HoGiaDinhs/{id.Value}/Members?page={page}&pageSize=4");
                    if (membersResponse.IsSuccessStatusCode)
                    {
                        var membersData = await membersResponse.Content.ReadAsStringAsync();
                        // Dùng JObject để tránh lỗi JValue khi binding động
                        var membersObj = JsonConvert.DeserializeObject<JObject>(membersData);

                        if (membersObj != null && membersObj["members"] != null)
                        {
                            var membersList = membersObj["members"].ToObject<List<CuDan>>() ?? new List<CuDan>();
                        ...
                        // Đọc số trang, trang hiện tại... dưới dạng int thuần
                        ViewBag.TotalMembers = (int?)(membersObj?["totalMembers"]) ?? 0;
                        ViewBag.CurrentPage = (int?)(membersObj?["currentPage"]) ?? 1;
                        ViewBag.TotalPages = (int?)(membersObj?["totalPages"]) ?? 1;
                        ViewBag.PageSize = (int?)(membersObj?["pageSize"]) ?? 4;
                    }
                    else
                    {
        ...
        public async Task<ActionResult> AddMember(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                {
                    var cuDanData = await cuDanResponse.Content.ReadAsStringAsync();
                    // API /CuDans trả về JSON dạng phân trang: { totalItems, page, pageSize, items }
                    var cuDanPage = JsonConvert.DeserializeObject<PhanTrangCuDan>(cuDanData);
                    var allCuDans = cuDanPage?.Items ?? new List<CuDan>();
        ...
        // 7. XÓA THÀNH VIÊN KHỎI HỘ (REMOVE MEMBER)
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RemoveMember(int id, int maHo)
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    // Bước 1: Lấy thông tin cư dân hiện tại
                    var getResponse = await client.GetAsync(apiBaseUrl + "/CuDans/" + id);
                    if (!getResponse.IsSuccessStatusCode)
                    {
                        TempData["ErrorMessage"] = "Không tìm thấy cư dân.";
                        return RedirectToAction("Details", new { id = maHo });
                    }

                    var data = await getResponse.Content.ReadAsStringAsync();
                    var cuDan = JsonConvert.DeserializeObject<CuDan>(data);

                    // Bước 2: Cập nhật thông tin (Gỡ khỏi hộ)
                    cuDan.MaHo = null;
                    cuDan.QuanHeVoiChuHo = ""; // Hoặc null tùy vào DB của bạn

                    // Bước 3: Gọi API PUT để lưu lại
                    var content = new StringContent(JsonConvert.SerializeObject(cuDan), Encoding.UTF8, "application/json");
                    var putResponse = await client.PutAsync(apiBaseUrl + "/CuDans/" + id, content);

                    if (putResponse.IsSuccessStatusCode)
                    {
                        // Đợi Trigger DB cập nhật lại số lượng thành viên
                        await Task.Delay(300);
                        TempData["SuccessMessage"] = "Đã xóa thành viên khỏi hộ.";
                    }
                    else
                    {
                        TempData["ErrorMessage"] = "Lỗi API: " + putResponse.ReasonPhrase;
                    }
                }
                catch (Exception ex)
                {
            ...
            // Quay lại trang chi tiết hộ gia đình
            return RedirectToAction("Details", new { id = maHo });
        }
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (resCuDan.IsSuccessStatusCode)
                    {
                        ...
                        var cuDanPage = JsonConvert.DeserializeObject<PhanTrangCuDan>(data);
                        var allCuDans = cuDanPage?.Items ?? new List<CuDan>();
