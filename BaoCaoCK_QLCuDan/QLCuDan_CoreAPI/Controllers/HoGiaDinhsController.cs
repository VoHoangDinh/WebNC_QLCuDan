...
namespace QLCuDan_CoreAPI.Controllers
{
    ...
    public class HoGiaDinhsController : ControllerBase
    {
        ...
        // GET: api/HoGiaDinhs/{id}/Members?page=1&pageSize=4
        [HttpGet("{id}/Members")]
        public async Task<ActionResult<object>> GetHoGiaDinhMembers(int id, [FromQuery] int page = 1, [FromQuery] int pageSize = 4)
        {
            var hoGiaDinh = await _context.HoGiaDinhs.FindAsync(id);
            if (hoGiaDinh == null) return NotFound();
